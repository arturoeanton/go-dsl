	...
	for i < len(lines) {
		...
		if strings.HasPrefix(line, "if ") && strings.HasSuffix(line, " then") {
			...
			for iterations < maxIterations {
				...
				// Handle continue (the iteration still counts toward the
				// safety limit, otherwise the cap could never trigger)
				if loopResult.ShouldContinue {
			...
			if iterations >= maxIterations {
				// Stop at the safety limit without failing the script.
				results = append(results, fmt.Sprintf("While loop stopped at safety limit (%d iterations)", maxIterations))
			}
