...
func (hd *HTTPDSLv3) setupGrammar() {
	...
	hd.dsl.Action("extractVariable", func(args []interface{}) (interface{}, error) {
		...
		// Extracting without a response is an error: silently setting an
		// empty variable hides broken scripts.
		if hd.engine.GetLastResponse() == "" {
			return nil, fmt.Errorf("no response available for extraction (variable $%s)", varName)
		}
	...
	// Most specific alternatives first (ordered choice): block forms ending
	// in "endif", then grouped forms, then the single-statement forms.
	...
	// Conditionals are lazy NodeActions: only the branch that is taken is
	// evaluated, so a non-taken branch can never run its side effects
	// (set/print/HTTP calls). The condition child is evaluated first.
	evalCond := func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (bool, error) {
		cond, err := ctx.Eval(n.Child(1))
		if err != nil {
			return false, err
		}
		return hd.toBool(cond), nil
	}

	hd.dsl.NodeAction("ifSimple", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		condition, err := evalCond(ctx, n)
		if err != nil {
			return nil, err
		}
		if condition {
			return ctx.Eval(n.Child(3)) // then statement only
		}
		return nil, nil
	})

	hd.dsl.NodeAction("ifElse", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		condition, err := evalCond(ctx, n)
		if err != nil {
			return nil, err
		}
		...
		return ctx.Eval(n.Child(5)) // else statement only
	})

	hd.dsl.NodeAction("ifBlock", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		condition, err := evalCond(ctx, n)
		if err != nil {
			return nil, err
		}
		if condition {
			return ctx.Eval(n.Child(3)) // block statements only when taken
		}
		return nil, nil
	})

	hd.dsl.NodeAction("ifElseBlock", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		condition, err := evalCond(ctx, n)
		if err != nil {
			return nil, err
		}
		if condition {
			return ctx.Eval(n.Child(3)) // then block only
		}
		return ctx.Eval(n.Child(5)) // else block only
	})

	hd.dsl.NodeAction("ifGrouped", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		condition, err := evalCond(ctx, n)
		if err != nil {
			return nil, err
		}
		if condition {
			return ctx.Eval(n.Child(4)) // statements inside ( ... )
		}
		return nil, nil
	})

	hd.dsl.NodeAction("ifGroupedElse", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		condition, err := evalCond(ctx, n)
		if err != nil {
			return nil, err
		}
		if condition {
			return ctx.Eval(n.Child(4)) // then statements
		}
		return ctx.Eval(n.Child(8)) // else statements
	})
	...
	// Loops are lazy NodeActions: the body node is re-evaluated on every
	// iteration (and the while condition is genuinely re-evaluated), so
	// statements execute once per iteration instead of once at parse time.
	hd.dsl.NodeAction("repeatLoop", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		times, _ := strconv.Atoi(n.Child(1).Token.Value)
		body := n.Child(4)
		...
		for i := 0; i < times; i++ {
			...
			if _, err := ctx.Eval(body); err != nil {
				return nil, err
			}
	...
	hd.dsl.NodeAction("whileLoop", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		...
		for iterations < maxIterations {
			// Re-evaluate the condition node on every iteration.
			condValue, err := ctx.Eval(condNode)
			if err != nil {
				return nil, err
			}
			if !hd.toBool(condValue) {
				break
			}
	...
	hd.dsl.NodeAction("foreachLoop", func(ctx *dslbuilder.EvalContext, n *dslbuilder.Node) (interface{}, error) {
		itemVar := strings.TrimPrefix(n.Child(1).Token.Value, "$")
		listVar := strings.TrimPrefix(n.Child(3).Token.Value, "$")
		body := n.Child(5)
